...
unsafe fn get_bits(mut bs: *mut bs_t, mut n: i32) -> uint32_t {
    ...
    let mut p: *const uint8_t = ((*bs).buf)
    ...
    loop {
        ...
        if get_bits(bs, 1 as i32) != 0 {
            ...
            (*gr)
                ...
                as usize] = get_bits(bs, 3 as i32) as uint8_t;
            (*gr)
            ...
            (*gr)
                .region_count[0 as i32
                as usize] = get_bits(bs, 4 as i32) as uint8_t;
            (*gr)
                .region_count[1 as i32
                as usize] = get_bits(bs, 3 as i32) as uint8_t;
            (*gr)
                .region_count[2 as i32 as usize] = 255 as i32 as uint8_t;
        }
        (*gr)
            .table_select[0 as i32
            as usize] = (tables >> 10 as i32) as uint8_t;
        (*gr)
        ...
        (*gr)
            ...
            as usize] = (tables & 31 as i32 as u32) as uint8_t;
        (*gr)
...
unsafe fn L3_decode_scalefactors(
    ...
    let mut scf_partition: *const uint8_t = (g_scf_partitions[(((*gr).n_short_sfb != 0)
    ...
    if (*gr).n_short_sfb != 0 {
        ...
        while i < 10 as i32 {
            iscf[(11 as i32 + i)
                ...
                + g_preamp[i as usize] as i32) as uint8_t;
            i += 1;
        }
    }
...
unsafe fn L3_huffman(
    ...
    let mut bs_next_ptr: *const uint8_t = ((*bs).buf)
        .offset(((*bs).pos / 8 as i32) as isize);
    let mut bs_cache: uint32_t = (*bs_next_ptr.offset(0 as i32 as isize)
        as u32)
                        ...
                        leaf = *codebook
                            .offset(
                                (bs_cache >> 32 as i32 - w)
                                    .wrapping_sub((leaf >> 3 as i32) as uint32_t)
                                    as isize,
                            ) as i32;
                    }
    ...
    loop {
        let mut codebook_count1: *const uint8_t = if (*gr_info).count1_table
            as i32 != 0
        {
            tab33.as_ptr()
        } else {
        ...
        if leaf_1 & 128 as i32 >> 0 as i32 != 0 {
            *dst
                .offset(
                    0 as i32 as isize,
                ) = if (bs_cache as int32_t) < 0 as i32 { -one } else { one };
            bs_cache <<= 1 as i32;
            bs_sh += 1 as i32;
        }
